...
# ===============================
# SAVE RESULT FOR WEBSITE (AUTO)
# ===============================

final_storm_value = float(np.mean(mean_dev_array))

# Create dataframe in expected format
web_df = pd.DataFrame({
    "storm_value": [final_storm_value]
})

# Build absolute path to Web forecast folder
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
WEB_FORECAST_PATH = os.path.join(
    BASE_DIR, "Web", "forecast", "output_pred.csv"
)

# Save CSV for PHP website
web_df.to_csv(WEB_FORECAST_PATH, index=False)

print("Website CSV updated at:")
print(WEB_FORECAST_PATH)
